...
// --- 1. PRECISE LAPLACIAN (Sum = 0) ---
  ...
  float dt = 0.2 * u_dt_scale;
  
  // Gravitational lensing: higher potential -> slower wave speed
  float potential = texture2D(u_gravity, v_texCoord).r;
  float k  = 1.2 / (1.0 + potential * potential * 20.0); // Speed boost modulated by gravity
